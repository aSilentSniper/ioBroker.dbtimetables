...
function toArray(value) {
	// fast-xml-parser liefert für ein leeres Element (<stations>\n</stations>) den String '' statt undefined
 ...
 *
 * @param {string|undefined} value IRIS-Zeitstempel im Format yyMMddHHmm
 * @returns {Date|null} das geparste Datum, oder null bei fehlendem/ungültigem Wert
 */
function parseIrisTime(value) {
	...
	if ([yy, month, day, hour, minute].some(n => Number.isNaN(n))) {
		return null;
	}
...
/** Schlanker Client für die DB Timetables (IRIS) API. */
class DbTimetablesClient {
	 ...
	 * @param {string} [options.baseUrl] abweichende API-Basis-URL (Standard: {@link DEFAULT_BASE_URL})
	 * @param {(msg: string) => void} [options.debug] optionaler Logger für Debug-Ausgaben
	 * @param {number} [options.timeoutMs] Timeout je HTTP-Request in Millisekunden (Standard: {@link DEFAULT_TIMEOUT_MS})
	 */
	...
	/**
	 * Führt einen GET-Request gegen die API aus und liefert den rohen Antworttext.
	 *
	 * @param path API-Pfad relativ zur Basis-URL
	 */
	async _get(path) {
		...
		try {
			...
			this.debug(`GET ${url} -> HTTP ${res.status}`);
			text = await res.text();
		} catch (err) {
			if (err && (err.name === 'TimeoutError' || err.name === 'AbortError')) {
				throw new Error(`Zeitüberschreitung nach ${Math.round(this.timeoutMs / 1000)}s für ${path}`);
			}
			// Node meldet Netzwerkfehler nur als "fetch failed", der eigentliche Grund steckt in err.cause
			const cause = err && err.cause && (err.cause.code || err.cause.message);
			throw new Error(`Netzwerkfehler für ${path}: ${cause || (err && err.message) || err}`);
		}
		if (!res.ok) {
			if (res.status === 401 || res.status === 403) {
				const detail = text.trim().slice(0, 200);
				throw new Error(
					`Authentifizierung fehlgeschlagen (HTTP ${res.status}) - Client-Id/API-Key prüfen${detail ? `: ${detail}` : ''}`,
				);
			}
			if (res.status === 404) {
	...
	/**
	 * Sucht eine Station anhand des Namensanfangs (ohne Beachtung der Groß-/Kleinschreibung),
	 * der EVA-Nummer oder des DS100-Codes. Die IRIS-API lieferte dabei in allen Tests nur eine Station
	 * und kennt keine Platzhalter: "Hannover Hbf" findet den Hbf, "Hannover" nur die erste Station
	 * mit diesem Namensanfang, "Pferdemarkt" gar nichts (die Station heißt "Langenhagen Pferdemarkt").
	 *
	 * @param {string} pattern Such-String (Namensanfang, EVA-Nummer oder DS100-Code)
	 * @returns {Promise<{name: string, eva: string, ds100: string}[]>} gefundene Stationen (leer, wenn nichts passt)
	 */
	 ...
	 *
	 * @param {string} eva EVA-Nummer der Station
	 ...
	 * @returns {Promise<object[]>} die nächsten Abfahrten, sortiert und auf `count` begrenzt
	 */
		...
		for (let h = 0; h < maxLookaheadHours; h++) {
			...
			for (const s of stops) {
				if (seenIds.has(s['@_id'])) {
					continue;
				} // Slices überlappen sich teilweise
				seenIds.add(s['@_id']);
				rawStops.push(s);
			}
	...
	/**
	 * Zählt, wie viele der zusammengeführten Abfahrten in der Zukunft liegen.
	 *
	 * @param rawStops rohe <s>-Objekte aus dem Sollfahrplan
	 * @param changesById Map von Stop-Id auf das rohe <s>-Änderungsobjekt
	 * @param now Referenzzeitpunkt
	 * @param categories falls gesetzt: nur diese Zuggattungen berücksichtigen
	 */
	...
	/**
	 * Führt Sollfahrplan und Änderungen zusammen und filtert nach Kategorie.
	 *
	 * @param rawStops rohe <s>-Objekte aus dem Sollfahrplan
	 * @param changesById Map von Stop-Id auf das rohe <s>-Änderungsobjekt
	 * @param categories falls gesetzt: nur diese Zuggattungen berücksichtigen
	...
	_mergeAndFilter(rawStops, changesById, categories) {
		...
		for (const s of rawStops) {
			...
			if (!dp) {
				continue;
			} // nur Ankunft, keine Abfahrt an dieser Station -> für Abfahrtstafel irrelevant
			const tl = s.tl || {};
			const category = tl['@_c'] || '';
			if (categories && !categories.includes(category.toUpperCase())) {
				continue;
			}
